...
##
def _rotate(firePopup, direction, magnitude=None, units='degrees'):
    # Put here to avoid circular dependencies. If put in global space, import error

    firePopup('info', 'Rotate', "Magnitude: " + str(magnitude) + ", direction: " + str(direction) + ", units: " + str(units))
    ...
    if direction == 'left' or direction == 'right':
        ...
        translationVector.scaleBy(magnitude*10)
...
def run(firePopup, direction, magnitude=None, units='degrees'):
    return _rotate(firePopup, direction, magnitude, units)
