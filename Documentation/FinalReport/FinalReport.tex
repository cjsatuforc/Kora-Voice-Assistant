...
Save As Command
\textit{saveAs()} first checks if the call is coming from \textit{save()}. If it is, then it creates a copy of the project and saves it as the supplied filename. Otherwise, \textit{saveAs()} first converts the supplied filename to camelcase and then creates a copy of the file and saves it under the camelcased filename.
