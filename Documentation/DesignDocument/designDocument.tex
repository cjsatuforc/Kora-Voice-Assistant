...
\section{Design Stakeholders and Concerns}
    ...
    \designConcernDef{Component Functional Responsibilities}{Developers}{What functionality is each component responsible for?}
...
\section{Design Viewpoint: Information}
	\subsection{Addressed Design Concerns}
		\begin{itemize}
			\item \designConcernRef{Log Files}
...
\section{Design Viewpoint: Interfaces}
    ...
    \subsection{Design View: User Interface}
        ...
        If the command succeeds, the application uses a voice synthesizer to alert  user that the previous command command succeeded.
        If the command fails, the voice synthesizer uses the voice synthesizer to alert the user that the previous command failed.
...
\section{Design Viewpoint: Interactions}
    ...
    \subsection{Design View: Module Interactions}
        The diagram below specifies the interactions that occur between modules within the application: 
		\begin{figure}[H]
			\includegraphics[width=1\textwidth, height=1\textheight]{ModuleInteractionsUML.eps}
			\centering
